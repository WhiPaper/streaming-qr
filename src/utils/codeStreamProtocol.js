/**
 * Code Stream Protocol
 *
 * Each barcode symbol contains a JSON object with:
...
export class CodeStreamProtocol {
  constructor() {
    this.streams = new Map(); // Map<streamId, StreamData>
  }

  /**
   * Parse a symbol payload string and extract stream data
   */
  parseSymbol(codeString) {
    try {
      ...
      return { error: "Failed to parse symbol: " + error.message };
    }
  }
  ...
  processChunk(codeString) {
    const parsed = this.parseSymbol(codeString);
    if (parsed.error) {
      return parsed;
    }

    const { streamId, sequence, total, data } = parsed;
...
/**
 * Helper function to create a symbol chunk from data
 * This is useful for generating test data
 */
export function createCodeChunk(streamId, sequence, total, chunkData) {
...
/**
 * Split data into chunks that fit in barcodes
 * Practical payload limit depends on the encoder being used
 */
export function splitDataIntoCodeChunks(data, maxChunkSize = 2000) {
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    chunks.push(createCodeChunk(streamId, i, totalChunks, chunkData));
  }
